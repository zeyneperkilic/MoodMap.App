...
with open(os.path.join(current_dir, "regression_weights.json"), "r") as f:
...
def log_weighted_check(cluster_id, intensity, songs):
    try:
        # Hangi yönde sıralama yapılacağı
        ascending = intensity < 5

        # Intensity'ye göre öğrendiğimiz regression ağırlıkları
        weights = regression_weights.get(str(intensity), regression_weights["5"])

        scores = []
        for song in songs:
            # Yeni, dinamik ağırlıklarla kontrol
            score = (
                weights["energy"]       * song.get('energy', 0) +
                weights["valence"]      * song.get('valence', 0) +
                weights["danceability"] * song.get('danceability', 0) +
                weights["tempo"]        * song.get('tempo', 0)
            )
            scores.append(score)
...
def recommend_songs(cluster_id: int, intensity: int = 5):
    try:
        ...
        if (filtered.empty
            ...
            fb["weighted_score"] = (
            weights["danceability"] * filtered.danceability +
            weights["energy"] * filtered.energy +
            weights["valence"] * filtered.valence +
            weights["tempo"] * filtered.tempo
)
